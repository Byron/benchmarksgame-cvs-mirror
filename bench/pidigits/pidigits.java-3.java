...
public class pidigits {
   final static int 
      // plain int values
      FOUR = 0, BQ = 1, BR = 2, BT = 3, 
      // GMP integers
      V = 4, ER1 = 5, Q1 = 6, R1 = 7, T1 = 8, U1 = 9, ES1 = 10, ER = 11, 
   ...
   int i;
   StringBuilder strBuf = new StringBuilder(1000000);
   final int n;
   ...
   public class exec implements Runnable
   {
     ...
     {
       _run();
       acquire( moreWork, 1 ); // leave the thread spinning until more work arrives
                               // - unparking takes ages on some boxes
     }
     ...
     public void _run()
     {
       ...
       // the while loop makes sure the thread doesn't get preempted
       // - don't care about the CPU going wild; it would be idle otherwise anyway
   ...
   // the latter is the same as computing quotient and remainder of ( q*4 + r )/( s*4 + t );
   // if the remainder is greater or equal to q,
   ...
   final exec[] COMPOSE_L = new exec[]{ 
      ...
      new exec( new exec[]{ new exec( ADD, ES1, U, R ),
         // DIV_Q_R is approx the same cost as two muls
         // so this splits the work roughly equally
         new exec( DIV_Q_R, ER, ER1, ES1, T ) } ), 
   ...
   private boolean multi_threaded_compute( exec[] code, int bq, int br, int bt, boolean compare )
   {
     ...
     {
       executor.execute( code[ i ] ); // we are one thread, so skip code[ 0 ], 
                                      // if comparing the remainder is needed
       moreWork.release();
     }
     ...
     // ER1 >= Q means the remainder of (4*q+r)/t contains q,
     // and the quotient is the same as (3*q+r)/t
     boolean r = GmpUtil.mpz_cmp( values[ ER1 ], values[ Q ] ) >= 0; 
   ...
   private boolean printDigit(int y)
   {
      strBuf.append((char)(y+'0'));
      if (++i % 10 == 0 || i == n) {
   ...
   void printAll()
   {
     ...
     // these are initially unavailable
     for( int i = 0; i < sema.length; ++i ) sema[ i ] = new Semaphore( 0 );
     ...
     for (;;) {
       if ( multi_threaded_compute( COMPOSE_L, k, 4*k+2, 2*k+1, true ) ) {
         ...
         // compare == false - computation is in background; foreground thread can print
         multi_threaded_compute( COMPOSE_R, 10, -10*y, 1, false ); 
 
         boolean r = printDigit(y);
         acquire( allDone,  COMPOSE_R.length ); // wait for the COMPOSE_R to complete

         if ( r ) {
           System.out.print( strBuf );
           System.out.flush();
           // don't have to be this nice in a one-shot run
           for( int i = V; i < values.length; ++i ) GmpUtil.mpz_clear( values[ i ] ); 
           return;
         }
       } else {
         long g = values[ T ];
   ...
   public static void main(String[] args){
      pidigits spigot = new pidigits(Integer.parseInt(args[0]));
      spigot.printAll();
      System.exit(0);
   }
}
