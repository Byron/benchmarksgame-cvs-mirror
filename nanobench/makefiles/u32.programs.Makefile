...
# $Id: u32.programs.Makefile,v 1.24 2016/11/29 22:09:41 igouy-guest Exp $
...
%.lisaac_run: %.li
	-$(LISAAC) -i 20 $(TEST)
	-$(GCC) $(STD_COPTS) $(COPTS) $(GCCOPTS) $(TEST).c -o $@
...
%.ghc_run: %.hs $(GHC)
	-$(GHC) --make -fllvm -O2 -XBangPatterns -rtsopts $(GHCOPTS) $< -o $@
...
%.clojure_run: %.clojure $(CLOJURE)
	-mv $< $(TEST).clj
	-$(JDKRUN) -Dclojure.compiler.direct-linking=true -Dclojure.compile.path=. -cp .:$(CLOJURE):$(CLOJURECONTRIB) clojure.lang.Compile $(TEST)
