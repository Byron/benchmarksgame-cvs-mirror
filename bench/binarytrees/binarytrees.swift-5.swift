...
func worker_thread(tdata:UnsafeMutablePointer<Void>) -> UnsafeMutablePointer<Void> {
   ...
   let iterations = data.pointee.iterations
   let depth = data.pointee.depth
   var check = 0
...
while depth <= maxDepth {
   ...
   pthread_create(&worker.pointee.pid, nil, worker_thread, worker)
   depth += 2
   worker += 1
}
...
for i in 0..<size {
   ...
   pthread_join(worker.pointee.pid,nil)
   print("\(worker.pointee.iterations*2)\t trees of depth \(worker.pointee.depth)\t check: \(worker.pointee.check)")
}
