...
class RecursiveSpectralNorm(size:Int){  
   ...
   def fastMatrixTimesVector(Mat: (Int,Int) => Double, vec:Array[Double]) = {
    var w:List[scala.actors.Future[Array[Double]]] = List()
    for(i <- 0 to (numProcessors -1))
      w = w ::: List(future{subMatrixTimesVector(Mat,chunkIndex(i),vec)})
   ...
   def computeNewUandV(u:Array[Double]) = {
      val w = fastMatrixTimesVector(A,u)
      fastMatrixTimesVector(At,w)
   }

   def updateUandV(u: Array[Double], v:Array[Double])
               : Tuple2[Array[Double], Array[Double]] = {
     val vnew = computeNewUandV(u)
     val unew = computeNewUandV(vnew)
     (unew,vnew)
   }
